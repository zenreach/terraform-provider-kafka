...
const (
	validDescribeResponse = `Topic:file-imported	PartitionCount:12	ReplicationFactor:3	Configs:
...
Topic: file-imported	Partition: 11	Leader: -1	Replicas: 0	Isr:`
	...
	noBrokersError = `Error while executing topic command : replication factor: 1 larger than available brokers: 0
[2016-03-14 15:19:38,777] ERROR kafka.admin.AdminOperationException: replication factor: 1 larger than available brokers: 0
 ...
 (kafka.admin.TopicCommand$)`
	...
	topicExistsError = `Error while executing topic command : Topic "test" already exists.
[2016-03-14 16:03:58,893] ERROR kafka.common.TopicExistsException: Topic "test" already exists.
 ...
 (kafka.admin.TopicCommand$)`
	...
	errorWithWarnings = `WARNING: If partitions are increased for a topic that has a key, the partition logic or ordering of the messages will be affected
...
func TestKafkaManagingClient_emptyTopicInfo(t *testing.T) {
	...
	if err == nil {
		t.Fatal("Error is expected, but success found. Sometimes success is not what you are after.")
	}
}
